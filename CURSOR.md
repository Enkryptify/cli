...
- `ek login` - Authenticate with a provider
    - **Enkryptify (default)**: `ek login` (no `--provider` needed)
    - **Other providers**: `ek login --provider <providerName>` (required for AWS, GCP)
    - Options: `-p, --provider <providerName>` (defaults to 'enkryptify'), `-f, --force` (force re-authentication)
    - Examples: `ek login`, `ek login --provider aws`, `ek login --provider gcp`
- `ek configure` (alias: `ek setup`) - Configure project settings
    - **Enkryptify (default)**: `ek configure` (no `--provider` needed)
    - **Other providers**: `ek configure --provider <provider>` (required for AWS, GCP)
    - Options: `--provider <provider>` (defaults to 'enkryptify')
    - Examples: `ek configure`, `ek configure --provider aws`, `ek configure --provider gcp`
- `ek run <command...>` - Run a command with secrets injected as environment variables
    - Options: `-e, --env <environmentName>` (overrides default from config)
    - Example: `ek run pnpm run dev` or `ek run -- pnpm run dev`
- `ek list` - List secrets in the current environment
    - Options: `-s, --show` (show secret values, default: masked)
- `ek create <name> [value]` - Create a new secret
    - `name`: Required, must match pattern `[A-Za-z0-9_-]+`
    - `value`: Optional, prompts if not provided
    - Example: `ek create MySecret "my value"`
- `ek update <name>` - Update a secret in the current environment
    - Options: `--ispersonal` (make secret personal, Enkryptify provider only)
    - Example: `ek update MySecret` or `ek update API_KEY --ispersonal`
- `ek delete <name>` - Delete a secret from the current environment
    - Example: `ek delete MySecret`

### Provider Setup Requirements

**Enkryptify (Default Provider):**
...
1. **Build Jobs** (parallel):
    ...
    - `build-windows`: Builds Windows binary (x64 only - Bun doesn't support Windows ARM64, x64 binary runs on ARM64 via emulation)
...
- **Note**: Windows ARM64 is not supported by Bun. Windows ARM64 users run the x64 binary via emulation.
