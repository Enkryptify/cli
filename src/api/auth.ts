...
export class Auth {
    ...
    private renderAuthPage(params: {
        ...
        const icon =
            state === "success"
                ? `<svg class="state" viewBox="0 0 64 64" width="56" height="56"><circle class="ring ring-ok" cx="32" cy="32" r="30"/><polyline class="check" points="21,34 29,42 43,25"/></svg>`
                : `<svg class="state" viewBox="0 0 64 64" width="56" height="56"><circle class="ring ring-err" cx="32" cy="32" r="30"/><line class="x1" x1="23" y1="23" x2="41" y2="41"/><line class="x2" x1="41" y1="23" x2="23" y2="41"/></svg>`;

        const autoCloseScript = autoClose ? `<script>setTimeout(function(){window.close()},3000)</script>` : "";
...
<link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@500&family=Source+Sans+3:wght@400;500&display=swap" rel="stylesheet"/>
<style>
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
:root{--bg:#0d0d0d;--fg:#f5f5f5;--muted:#8a8a8a;--line:rgba(255,255,255,.06);--ok:#2ac769;--err:#ff5e5e}
body{font-family:'Source Sans 3',system-ui,-apple-system,sans-serif;background:var(--bg);color:var(--fg);min-height:100dvh;display:flex;align-items:center;justify-content:center;padding:24px;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}
.content{display:flex;flex-direction:column;align-items:center;text-align:center;gap:28px;max-width:360px;opacity:0;transform:translateY(6px);animation:rise .6s .1s cubic-bezier(.16,1,.3,1) forwards}
@keyframes rise{to{opacity:1;transform:none}}
.logo{display:block;height:23px;width:auto}
.icon{display:flex}
.state{display:block}
.ring{fill:none;stroke-width:2}
.ring-ok{stroke:var(--ok);stroke-dasharray:189;stroke-dashoffset:189;animation:draw .65s .3s cubic-bezier(.4,0,.2,1) forwards}
.ring-err{stroke:var(--err);opacity:.28}
.check{fill:none;stroke:var(--ok);stroke-width:2.5;stroke-linecap:round;stroke-linejoin:round;stroke-dasharray:36;stroke-dashoffset:36;animation:draw .35s .82s cubic-bezier(.4,0,.2,1) forwards}
.x1,.x2{fill:none;stroke:var(--err);stroke-width:2.5;stroke-linecap:round;stroke-dasharray:26;stroke-dashoffset:26}
...
h1{font-size:18px;font-weight:500;letter-spacing:-.01em}
.desc{font-size:14px;color:var(--muted);line-height:1.55}
.hint{font-family:'JetBrains Mono',ui-monospace,monospace;font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:var(--muted)}
@media (prefers-reduced-motion:reduce){.content{opacity:1;transform:none;animation:none}.ring-ok,.check,.x1,.x2{animation:none;stroke-dashoffset:0}}
</style>
</head>
<body>
<div class="content">
    <span class="logo">${this.logoSvg()}</span>
...
${autoCloseScript}
    ...
    private authErrorResponse(message: string): Response {
        const html = this.renderAuthPage({
            state: "error",
            documentTitle: "Sign-in failed — Enkryptify",
            title: "We couldn't sign you in",
            subtitle: this.escapeHtml(message),
            hint: "Close this tab and try again from your terminal",
        });
    ...
    private authSuccessResponse(): Response {
        const html = this.renderAuthPage({
            state: "success",
            documentTitle: "Signed in — Enkryptify",
            title: "You're all set",
            subtitle: "Signed in to Enkryptify. Head back to your terminal.",
            hint: "This tab will close automatically",
            autoClose: true,
        });
